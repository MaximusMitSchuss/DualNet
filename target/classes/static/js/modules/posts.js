...
function renderCommentsTree(comments) {
  return comments.map(c => `\n    <div class="comment" data-cid="${c.id}">\n      <div class="comment-head"><strong>${escapeHtml(c.authorUsername)}</strong> <time class="comment-time">${c.createdAt ? escapeHtml(new Date(c.createdAt).toLocaleString()) : ''}</time></div>\n      <div class="comment-body">${escapeHtml(c.text)}</div>\n      <div class="comment-actions"><button class="comment-reply" data-author="${escapeHtml(c.authorUsername)}" data-cid="${c.id}" title="Antworten"><span class="msg">💬</span></button></div>\n      ${c.children && c.children.length ? `<div class="replies">${renderCommentsTree(c.children)}</div>` : ''}\n    </div>`).join('\n');
}
...
function renderPostHtml(p) {
  ...
  const commentsHtml = renderCommentsTree(tree);
...
export function renderPosts(postsArr, container) {
      ...
      commentBtn.addEventListener('click', () => {
        ...
        send.addEventListener('click', async () => {
          ...
          try {
            ...
            // append to comments
            const newEl = createCommentElement(c);
            commentsDiv.appendChild(newEl);
            // wire reply handler for the newly added comment
            attachReplyHandlerToElement(newEl, id, commentsDiv);
            node.remove();
          } catch (e) {
  ...
  container.querySelectorAll('.post').forEach(el => {
    ...
    commentsDiv.querySelectorAll('.comment').forEach(cEl => {
      attachReplyHandlerToElement(cEl, postId, commentsDiv);
    });
  });
}
...
function attachReplyHandlerToElement(commentEl, postId, commentsDiv) {
  ...
  replyBtn.addEventListener('click', () => {
    ...
    send.addEventListener('click', async () => {
      ...
      try {
        ...
        attachReplyHandlerToElement(newEl, postId, commentsDiv);
        form.remove();
      } catch (e) {
